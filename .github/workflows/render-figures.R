######################################################################################################
# This file renders the figures for the `digitTests` readme (https://github.com/koenderks/digitTests).
...
p <- ggplot2::ggplot(plotData, ggplot2::aes(x = date, y = count)) +
  ggplot2::geom_bar(stat = 'identity', fill = rgb(50, 205, 135, maxColorValue = 255),
                    col = 'black', size = 1) +
  ...
  ggplot2::labs(title = 'Monthly CRAN downloads since August 2021',
...
prevData <- read.csv(paste0(dirname(getwd()), '/digitTests/man/figures/readme/worldmap/downloads.csv'))
...
dnldata <- installr::read_RStudio_CRAN_data(dnldata_folder, packages = 'digitTests', use_data_table = TRUE)

# Merge new data with previously stored data and save the merged data set
data <- rbind(prevData, dnldata)

write.csv(x = data,
          file = paste0(dirname(getwd()), '/digitTests/man/figures/readme/worldmap/downloads.csv'),
          row.names = FALSE)
...
world<-maptools::readShapePoly(fn = paste0(dirname(getwd()),
                                           '/digitTests/man/figures/readme/worldmap/ne_110m_admin_0_countries'))
